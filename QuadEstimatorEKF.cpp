...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  ekfState(6) = AngleNormF(q_t_bar.Yaw());
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) = predictedState(3) + accel2.x * dt;
  predictedState(4) = predictedState(4) + accel2.y * dt;
  predictedState(5) = predictedState(5) + (accel2.z - 9.81f) * dt;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  VectorXf u_3(3);
  u_3 << accel.x, accel.y, accel.z;
  VectorXf RbgP_udt(3);
  RbgP_udt = RbgPrime * u_3 * dt;

  for (int i = 0; i < 3; i++)
  {
    gPrime(i, i+3) = dt;
    gPrime(i+3, 6) = RbgP_udt(i);
  }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  zFromX(0) = ekfState(6);
  zFromX(0) = z(0) - AngleNormF(z(0) - zFromX(0));
